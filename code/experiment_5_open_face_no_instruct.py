...
from lab_pipeline.datasets import FaceDatasetAdapter
...
SYSTEM_PROMPT = 'You are an expert at classifying sentiment from facial expressions. Output format: Respond with exactly one label: Positive, Negative, or Neutral. Output the label only, no explanation, punctuation, or additional text.'
...
def create_config() -> PipelineConfig:
    ...
    immutable = {
        "experiment": "5.0-open-face-no-instruction",
    ...
    return PipelineConfig(
        ...
        experiment_name="5.0-open-face-no-instruction",
        provider=PROVIDER,
        model=MODEL,
...
async def run() -> int:
    ...
    try:
        ...
        # )
        dataset = FaceDatasetAdapter(face_dir.expanduser().resolve())
